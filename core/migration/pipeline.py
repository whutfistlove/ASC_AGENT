...
from core.analysis.migration_context import build_migration_context_pack
...
from core.knowledge.example_retrieval import select_header_examples
from core.knowledge.knowledge_base import load_knowledge_base
from core.llm.agent_tools import build_toolbox
from core.llm.best_of_n import best_of_n, score_header_code
from core.llm.model_client import (
    BaseModelClient,
    extract_json_object,
    normalize_generated_text,
)
from core.migration.fix_once import run_single_fix_from_log
from core.testing.host_shim_rule import check_host_stdlib_forwarding, is_host_stdlib_shim
from core.testing.verify_includes import verify_header_self_contained

# 「已验证、可安全跳过」的状态集合，单一事实源在 core.analysis.migration_state，
# 与 package_planner.SAFE_STATUSES / test_migrator 的依赖就绪集合共用同一份定义。
SAFE_DEPENDENCY_SKIP_STATUSES = VALIDATED_STATUSES
...
class DependencyAwareHeaderResult:
    ...
    host_shim_check: dict = field(default_factory=dict)
...
def _reverse_dep_map(dep_map: dict[str, list[str]], universe: set[str]) -> dict[str, list[str]]:
    """反向邻接表（仅限 `universe` 内的边）：dep -> [依赖它的 header...]。

    在闭包迁移入口构建一次，供多次失败延期复用，避免每次失败都重扫整个 universe。
    """
    reverse: dict[str, list[str]] = {}
    for header in universe:
        for dep in dep_map.get(header, []):
            if dep in universe:
                reverse.setdefault(dep, []).append(header)
    return reverse


def _transitive_dependents(target: str, reverse_adj: dict[str, list[str]]) -> set[str]:
    """返回所有（传递地）依赖 `target` 的 header（沿预建反向邻接表 BFS，O(V+E)）。

    用于「局部可用、整体延期」：某 leaf 失败时，只跳过真正需要它的下游，独立分支照常推进。
    `reverse_adj` 已限定在闭包 universe 内，故结果自然落在 universe 中。
    """
    ...
    while queue:
        ...
        for dependent in reverse_adj.get(node, []):
            if dependent != target and dependent not in dependents:
...
def _manual_bootstrap_cover(header: str, config: Config) -> str | None:
    """手写一次、长期复用的基础设施头（策略由 reference 注入到 config.migration_policy）。"""
...
"""
    ...
    def convert_dependency_closure(
        ...
        *,
        ...
        The order is leaf-first according to `core.analysis.dep_graph`. Headers that
        ...
        # 反向邻接表只构建一次，多次失败延期复用（旧实现每次失败都 O(V·E) 重扫 universe）。
        closure_reverse = _reverse_dep_map(closure_dep_map, closure_universe)
        ...
        def _register_failure(failed_header: str) -> bool:
            ...
            if defer_dependents_on_failure:
                for dependent in _transitive_dependents(failed_header, closure_reverse):
        ...
        for header in ordered_headers:
            ...
            try:
                ...
                # __host_stdlib 转发垫片内容门：空壳能过自包含编译，但漏掉 host 侧 #include <X>
                # 是迁移缺陷（host 侧拿不到系统类型）。纯词法、不依赖编译器，故总是运行；
                # 是否按失败处理沿用 verify_includes_strict。
                if write_to_repo and is_host_stdlib_shim(run_result.target_relpath):
                    target_file = Path(self.config.target_repo) / run_result.target_relpath
                    shim = check_host_stdlib_forwarding(
                        run_result.target_relpath,
                        target_file.read_text(encoding="utf-8"),
                    )
                    item.host_shim_check = shim.to_dict()
                    if shim.applicable and not shim.ok:
                        self._log(f"[host-shim] {header}: {shim.reason}")
                        if verify_includes_strict:
                            item.action = "failed_host_shim_check"
                            item.reason = shim.reason
                            if _register_failure(header):
                                result.error = f"{header}: host-stdlib 转发垫片校验失败（{shim.reason}）"
                                self._log(f"[verify-fail] {result.error}")
                                return result
                            continue  # 已按策略延期下游：不再对该头跑测试
    ...
    def _run_inner(self, input_path: Path, result: RunResult) -> RunResult:
        ...
        rewritten_target = cfg.model_output_dir / "rewritten_target.h"
        ...
        for r in range(1, max_rounds + 1):
            ...
            if current_style_passed:
                ...
                fixed_target = cfg.fix_output_dir / "post_hook_baseline.h"
            else:
...
class FakeVerifier:
    ...
    def __init__(self, config: Config, rounds_to_pass: int = 0, verbose: bool = False):
        ...
        self.verbose = verbose
        self._fix = config.fix_output_dir   # post_hook_baseline*
        self._repo = config.repo_log_output_dir  # git_commit*/git_push 日志
    ...
    def _save_baseline(self, content: str, extra_name: str) -> str:
        baseline_path = self._fix / "post_hook_baseline.h"
        save_text(baseline_path, content)
        save_text(self._fix / extra_name, content)
    ...
    def first_commit_verify(self, generated_file_path: Path, target_relpath: str) -> dict:
        ...
        save_text(self._repo / "git_commit.log", log)
    ...
    def repair_commit_verify(self, generated_file_path: Path, target_relpath: str, round_index: int) -> dict:
        ...
        log = self._checks_log(license_ok=True, style_ok=style_ok)
        save_text(self._repo / f"git_commit_round{round_index}.log", log)
    ...
    def push(self, branch_name: str) -> dict:
        save_text(self._repo / "git_push.log", f"[fake] pushed {branch_name}\n")
