...
import json
import re  # noqa: F401  (used in stringized type annotations)
...
def _entry(
    path: Path, test_root: Path, *, kind: str, known_headers: set[str],
    include_re: "re.Pattern[str] | None" = None, namespace: str = DEFAULT_INCLUDE_NAMESPACE,
) -> CCCLTestIndexEntry:
    relative_path = path.relative_to(test_root).as_posix()
    includes = parse_cuda_std_includes(path.read_text(encoding="utf-8", errors="replace"), include_re)
    candidates, unknown = _candidate_headers(includes, known_headers, namespace)
...
def _build_mappings(
    tests: list[CCCLTestIndexEntry], namespace: str = DEFAULT_INCLUDE_NAMESPACE
) -> list[HeaderTestMapping]:
    ns = namespace.strip("/")
...
def scan_test_index(
    ...
    *,
    ...
    # 命名空间随头根（cuda/std 或 cuda）；测试文件按同一前缀解析 include。
    ...
    for path in sorted(p for p in test_root.rglob("*") if p.is_file() and not is_env_file(p)):
        entry_kwargs = {"known_headers": known_headers, "include_re": include_re, "namespace": namespace}
        kind = classify_test_kind(path)
        if kind is not None:
            tests.append(_entry(path, test_root, kind=kind, **entry_kwargs))
        elif is_helper_header(path):
            helper_headers.append(_entry(path, test_root, kind="helper_header", **entry_kwargs))

    mappings = _build_mappings(tests, namespace)
