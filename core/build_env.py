...
def cann_bin_candidates() -> list[Path]:
    ...
    if home:
        cands += [Path(home) / "bin", Path(home) / "tools" / "ccec_compiler" / "bin"]
    for base in ("/usr/local/Ascend/cann", "/usr/local/Ascend/ascend-toolkit"):
        cands += [
            Path(base) / "bin",
            Path(base) / "x86_64-linux" / "bin",
            Path(base) / "x86_64-linux" / "ccec_compiler" / "bin",
            Path(base) / "tools" / "ccec_compiler" / "bin",
        ]
    return cands
