...
_VALID_HOST = (
    ...
    "int main(){ if (ascend::std::foo(1) != 1) ++g_failures; return g_failures == 0 ? 0 : 1; }\n"
)
_VALID_SPEC = {
    "gm_inputs": 1,
    "input_init": "h_x[i] = static_cast<float>(i);",
    "element_op_code": "z_val = ascend::std::foo(x_val);",
    "golden_code": "expected = x_ref;",
}
_VALID_ACCL_HEADER = (
    "#ifndef LIBASCENDCXX_INCLUDE_ASCEND_STD___ALGORITHM_FOO_H_\n"
    "#define LIBASCENDCXX_INCLUDE_ASCEND_STD___ALGORITHM_FOO_H_\n"
    "_ASCEND_STD_BEGIN\n"
    "template <class T> _ASCEND_AICORE_FN constexpr T foo(T x){ return x; }\n"
    "_ASCEND_STD_END\n"
    "#endif\n"
)
...
def _make_project(tmp_path, *, host=_VALID_HOST, spec=_VALID_SPEC,
                  accl_header=_VALID_ACCL_HEADER, with_test=True) -> Config:
    ...
    (accl / "libascendcxx/include/ascend/std/__algorithm").mkdir(parents=True)
    (accl / "libascendcxx/include/ascend/std/__algorithm/foo.h").write_text(accl_header, encoding="utf-8")
    (tmp_path / "examples" / "headers").mkdir(parents=True)
    (tmp_path / "examples" / "tests").mkdir(parents=True)
    if with_test:
        (cccl / "libcudacxx/test/std/__algorithm").mkdir(parents=True)
        (cccl / "libcudacxx/test/std/__algorithm/foo.pass.cpp").write_text("// cccl foo test\n", encoding="utf-8")
        host_dir = accl / "libascendcxx/test/libascendcxx/ascend/host"
        host_dir.mkdir(parents=True)
        (host_dir / "foo_tests.cpp").write_text(host, encoding="utf-8")
        kdir = accl / "libascendcxx/test/libascendcxx/ascend/kernel/foo_example"
...
def test_promote_overwrite_updates(tmp_path):
    cfg = _make_project(tmp_path)
    promote_operator(cfg, "foo")
    new_header = _VALID_ACCL_HEADER.replace("return x;", "return x + T(0);")
    (Path(cfg.accl_repo) / "libascendcxx/include/ascend/std/__algorithm/foo.h").write_text(
...
def test_quality_gate_skips_bad_host_test_but_keeps_header(tmp_path):
    ...
    bad_host = '#include "ascend/std/__algorithm/foo.h"\nint main(){ bool ok=false; return 0; }\n'
