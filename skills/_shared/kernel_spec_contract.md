...
- `golden_code` **绝不能调用 `asc::std::*`**，必须是独立参考实现。
...
3. `element_op_code`：用算子真实形态计算输出。二元返回值 `z_val = asc::std::op(x_val, y_val);`；原地 void（swap）`float a=x_val,b=y_val; asc::std::swap(a,b); z_val=a;`；三参（clamp）用常量边界 `z_val = asc::std::clamp(x_val, 10.0f, 100.0f);`；多输入多输出形态分别给 `out0_val...outM_val` 赋值。
