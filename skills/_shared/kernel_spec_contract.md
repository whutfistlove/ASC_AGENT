...
规则：
0. `dtype`（可选，默认 `float`）：脚手架按此标量类型生成整条流水线。浮点算子用 `float`/`double`（容差 1e-5 比对）；**整数语义算子（gcd/lcm 等）必须填 `int32_t` 或 `int64_t`**（脚手架改用精确相等比对），否则用 float 测整数算子语义上是错的。支持集合：float / double / int32_t / int64_t / int16_t。
