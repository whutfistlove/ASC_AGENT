...
3. 每个用例的 expected 必须是**独立**写死的值或独立公式，**不得**再调用 `asc::std::<algo>` 来产生 expected（否则永远自洽假绿）。
...
6. 按算子形态选择测法：二元返回值（max/min/clamp）用 `expect_eq("op(...)", asc::std::op(...), <独立期望>)`；原地 void（swap）先准备左值再调用 `asc::std::swap(a,b)`，然后校验 a、b 的新值。
