...
4. kernel 仿真测试（`ascend/kernel/<algo>_example`）在 `__aicore__` kernel 中逐元素
   ...
   且不依赖异常或动态内存。
5. 不要为通过测试而改变算子数学语义。**算子的 CCCL 语义是基准**：若失败根因是测试本身写错
   （例如把 `void`/原地算子（如 swap）当成“二元返回值”来用、把右值绑定到非 const 左值引用、
   把 `void` 赋给 `float`），请**保持算子签名/返回/语义正确**，在 `notes` 写明“需要修正的是测试”，
   此时 `rewritten_code` 可与基线保持一致（本流程只输出 header，无法直接改测试）。绝不可把算子
   改成错误形态去迁就错误的测试。
