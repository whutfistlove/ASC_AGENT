// auto-workload=fast (n=64, cores=1, inputs=2, outputs=1, dtype=int32_t)
#include "acl/acl.h"
#include "host.h"
#include "ascend/std/__numeric/midpoint.h"
#include <cmath>
#include <cstdlib>
#include <iostream>
#include <vector>
...
int main()
{
    ...
    for (size_t i = 0; i < n; ++i) {
        { if (i % 4 == 0) { h_in0[i] = INT32_MIN; h_in1[i] = INT32_MAX; } else if (i % 4 == 1) { h_in0[i] = INT32_MAX; h_in1[i] = INT32_MIN; } else { h_in0[i] = static_cast<int32_t>((static_cast<int32_t>(i % 97)) - 48); h_in1[i] = static_cast<int32_t>((static_cast<int32_t>((i * 3) % 101)) - 50); } }
    }
    ...
    ascend_std_midpoint_do(1, stream, static_cast<uint8_t*>(d_in0), static_cast<uint8_t*>(d_in1), static_cast<uint8_t*>(d_out0));
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        (void)expected;
        (void)expected0;
        { int64_t a = static_cast<int64_t>(x_ref); int64_t b = static_cast<int64_t>(y_ref); expected = static_cast<int32_t>(a + (b - a) / 2); }
        int32_t got0 = h_out0[i];
        if (print_samples < 0 || static_cast<long>(i) < print_samples) {
            std::cout << "[kernel][midpoint][" << i << "]" << " in0=" << in0_ref << " in1=" << in1_ref
    ...
    std::cout << "[kernel][midpoint] checked " << n << " elements, mismatches "
              << mismatches << std::endl;
    if (mismatches != 0) {
        aclrtFree(d_in0);
        aclrtFree(d_in1);
