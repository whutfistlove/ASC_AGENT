// auto-workload=fast (n=64, cores=1, inputs=3, outputs=2, dtype=int32_t)
#include "acl/acl.h"
#include "host.h"
#include "ascend/std/__numeric/spread3.h"
#include <cmath>
#include <cstdlib>
#include <iostream>
#include <vector>
...
int main()
{
    ...
    std::vector<int32_t> h_in2(n);
    std::vector<int32_t> h_out0(n);
    std::vector<int32_t> h_out1(n);
    auto& h_x = h_in0;
    auto& h_y = h_in1;
    (void)h_x;
    (void)h_y;
    for (size_t i = 0; i < n; ++i) {
        { h_in0[i] = (i * 7 + 3) % 11 - 5; h_in1[i] = (i * 13 + 1) % 11 - 5; h_in2[i] = (i * 19 + 5) % 11 - 5; }
    }
    ...
    ascend_std_spread3_do(1, stream, static_cast<uint8_t*>(d_in0), static_cast<uint8_t*>(d_in1), static_cast<uint8_t*>(d_in2), static_cast<uint8_t*>(d_out0), static_cast<uint8_t*>(d_out1));
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        int32_t expected1 = (int32_t)0;
        int32_t& expected = expected0;
        (void)expected;
        (void)expected0;
        (void)expected1;
        { int32_t lo = in0_ref < in1_ref ? (in0_ref < in2_ref ? in0_ref : in2_ref) : (in1_ref < in2_ref ? in1_ref : in2_ref);
int32_t hi = in0_ref > in1_ref ? (in0_ref > in2_ref ? in0_ref : in2_ref) : (in1_ref > in2_ref ? in1_ref : in2_ref);
expected0 = lo;
expected1 = hi - lo; }
        int32_t got0 = h_out0[i];
        if (print_samples < 0 || static_cast<long>(i) < print_samples) {
            std::cout << "[kernel][spread3][" << i << "][out0]" << " in0=" << in0_ref << " in1=" << in1_ref << " in2=" << in2_ref
        ...
        int32_t got1 = h_out1[i];
        if (print_samples < 0 || static_cast<long>(i) < print_samples) {
            std::cout << "[kernel][spread3][" << i << "][out1]" << " in0=" << in0_ref << " in1=" << in1_ref << " in2=" << in2_ref
    ...
    std::cout << "[kernel][spread3] checked " << n << " elements, mismatches "
              << mismatches << std::endl;
    if (mismatches != 0) {
        aclrtFree(d_in0);
        aclrtFree(d_in1);
