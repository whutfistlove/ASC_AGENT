#!/bin/bash
# Kernel simulation test for sort3.
...
# Ensure CANN's llvm-objdump (needed by extract_host_stub.py) is on PATH,
# even when ASCEND_HOME_PATH was preset and set_env.sh got skipped above.
if ! command -v llvm-objdump >/dev/null 2>&1; then
    for d in "$ASCEND_HOME_PATH/tools/ccec_compiler/bin" "$ASCEND_HOME_PATH/bin" \
             /usr/local/Ascend/cann/x86_64-linux/ccec_compiler/bin \
             /usr/local/Ascend/cann/x86_64-linux/bin; do
        if [ -x "$d/llvm-objdump" ]; then export PATH="$d:$PATH"; break; fi
    done
fi
...
echo "kernel simulation for sort3 finished."
