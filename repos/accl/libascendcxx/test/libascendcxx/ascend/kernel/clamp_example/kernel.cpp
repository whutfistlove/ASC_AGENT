// auto-workload=fast (n=64, cores=1, tiles=1x64, inputs=1, outputs=1, dtype=float)
#include "kernel_operator.h"
#include "ascend/std/__algorithm/clamp.h"

extern "C" __global__ __aicore__ void clamp_kernel(GM_ADDR in0_gm, GM_ADDR out0_gm)
{
    ...
    for (int32_t tile = 0; tile < TILE_NUM; ++tile) {
        ...
        for (int32_t i = 0; i < TILE_SIZE; ++i) {
            ...
            (void)z_val;
            (void)out0_val;
            { z_val = ascend::std::clamp(x_val, 10.0f, 100.0f); }
