// auto-workload=fast (n=64, cores=1, inputs=1, outputs=1, dtype=float)
#include "acl/acl.h"
#include "host.h"
...
int main()
{
    ...
    ascend_std_clamp_do(1, stream, static_cast<uint8_t*>(d_in0), static_cast<uint8_t*>(d_out0));
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        if (print_samples < 0 || static_cast<long>(i) < print_samples) {
            std::cout << "[kernel][clamp][" << i << "]" << " in0=" << in0_ref
