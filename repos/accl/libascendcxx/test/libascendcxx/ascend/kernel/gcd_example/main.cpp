...
#include "acl/acl.h"
#include "host.h"
#include "ascend/std/__numeric/gcd.h"
#include <cmath>
#include <cstdlib>
#include <iostream>
#include <vector>
...
int main()
{
    ...
    for (size_t i = 0; i < n; ++i) {
        { h_in0[i] = static_cast<int32_t>((static_cast<int32_t>(i % 37)) - 18); h_in1[i] = static_cast<int32_t>((static_cast<int32_t>((i * 5) % 41)) - 20); }
    }
    ...
    ascend_std_gcd_do(1, stream, static_cast<uint8_t*>(d_in0), static_cast<uint8_t*>(d_in1), static_cast<uint8_t*>(d_out0));
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        (void)expected;
        (void)expected0;
        { int32_t a = x_ref; int32_t b = y_ref; if (a < 0) { a = -a; } if (b < 0) { b = -b; } while (b != 0) { int32_t t = static_cast<int32_t>(a % b); a = b; b = t; } expected = a; }
