#include "ascend/std/__algorithm/any_of.h"
...
int main()
{
    ...
    expect_bool(
        "any_of evens",
        ascend::std::any_of(evens, evens + 4, IsEven()),
        independent_any_of(evens, IsEven()));
    expect_bool(
        "any_of mixed even",
        ascend::std::any_of(mixed, mixed + 4, IsEven()),
        independent_any_of(mixed, IsEven()));
    expect_bool(
        "any_of odds even",
        ascend::std::any_of(odds, odds + 4, IsEven()),
        independent_any_of(odds, IsEven()));
    expect_bool(
        "any_of negative positive",
        ascend::std::any_of(negative, negative + 4, IsPositive()),
        independent_any_of(negative, IsPositive()));
    expect_bool(
        "any_of empty range",
        ascend::std::any_of(empty, empty, IsEven()),
        false);

    constexpr int constexpr_values[] = {1, 3, 4, 7};
    static_assert(ascend::std::any_of(constexpr_values, constexpr_values + 4, IsEven()), "");
