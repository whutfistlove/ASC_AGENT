...
void check_pair(A a, B b)
{
    ...
    static_assert(std::is_same<ExpectedType, decltype(ascend::std::gcd(a, b))>::value, "");
    const ExpectedType expected = gcd_ref(a, b);
    expect_eq("gcd(a,b)", ascend::std::gcd(a, b), expected);
    expect_eq("gcd(b,a)", ascend::std::gcd(b, a), gcd_ref(b, a));
}
...
int main()
{
    ...
    constexpr auto c = ascend::std::gcd(48, -18);
    static_assert(c == 6, "");
