#include "ascend/std/__algorithm/all_of.h"
...
int main()
{
    ...
    expect_bool(
        "all_of positive",
        ascend::std::all_of(positive, positive + 4, IsPositive()),
        independent_all_of(positive, IsPositive()));
    expect_bool(
        "all_of mixed positive",
        ascend::std::all_of(mixed, mixed + 4, IsPositive()),
        independent_all_of(mixed, IsPositive()));
    expect_bool(
        "all_of evens",
        ascend::std::all_of(evens, evens + 4, IsEven()),
        independent_all_of(evens, IsEven()));
    expect_bool(
        "all_of empty range",
        ascend::std::all_of(empty, empty, IsPositive()),
        true);

    constexpr int constexpr_values[] = {2, 4, 6};
    static_assert(ascend::std::all_of(constexpr_values, constexpr_values + 3, IsEven()), "");
