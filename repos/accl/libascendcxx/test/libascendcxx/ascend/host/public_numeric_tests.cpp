...
int main()
{
    ...
    if (ascend::std::gcd(-48, 18) != gcd_ref(-48, 18)) {
        ++failures;
        std::cout << "[host][public_numeric] gcd failed" << std::endl;
    }
    if (ascend::std::lcm(-12, 18) != lcm_ref(-12, 18)) {
        ++failures;
        std::cout << "[host][public_numeric] lcm failed" << std::endl;
    }
    if (ascend::std::midpoint(3, 8) != midpoint_ref(3, 8)) {
        ++failures;
        std::cout << "[host][public_numeric] midpoint odd failed" << std::endl;
    }
    if (ascend::std::midpoint((std::numeric_limits<int>::min)(), (std::numeric_limits<int>::max)()) !=
    ...
    if (ascend::std::midpoint(values, values + 4) != values + 2) {
        ++failures;
        std::cout << "[host][public_numeric] pointer midpoint failed" << std::endl;
    }

    constexpr int cgcd = ascend::std::gcd(84, 30);
    constexpr int clcm = ascend::std::lcm(12, 18);
    constexpr int cmid = ascend::std::midpoint(10, 20);
