...
void check_pair(A a, B b)
{
    ...
    static_assert(std::is_same<ExpectedType, decltype(ascend::std::lcm(a, b))>::value, "");
    expect_eq("lcm(a,b)", ascend::std::lcm(a, b), lcm_ref(a, b));
    expect_eq("lcm(b,a)", ascend::std::lcm(b, a), lcm_ref(b, a));
}
...
int main()
{
    ...
    (void)ascend::std::lcm((std::numeric_limits<int>::min)(), 2UL);

    constexpr auto c = ascend::std::lcm(21, -6);
    static_assert(c == 42, "");
