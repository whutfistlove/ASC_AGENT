...
extern "C" __global__ __aicore__ void generate_kernel(GM_ADDR in0_gm, GM_ADDR out0_gm)
{
    ...
    for (int32_t tile = 0; tile < TILE_NUM; ++tile) {
        ...
        for (int32_t i = 0; i < TILE_SIZE; ++i) {
            ...
            (void)z_val;
            (void)out0_val;
            { float buf[1] = {in0_val}; asc::std::generate(buf, buf + 1, +[]() -> float { return 42.0f; }); out0_val = buf[0]; }
