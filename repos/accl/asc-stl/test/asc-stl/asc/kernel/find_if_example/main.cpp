...
int main()
{
    ...
    for (size_t i = 0; i < n; ++i) {
        { h_x[i] = static_cast<int32_t>((i % 7) * 2); }
    }
    ...
    asc_std_find_if_do(8, stream, static_cast<uint8_t*>(d_in0), static_cast<uint8_t*>(d_out0));
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        (void)expected;
        (void)expected0;
        { int32_t arr[4] = {2, 4, in0_ref, 8};
int32_t idx = 3;
for (int k = 0; k < 4; ++k) {
    if (arr[k] > 5) { idx = k; break; }
}
expected0 = idx; }
        int32_t got0 = h_out0[i];
        if (print_samples < 0 || static_cast<long>(i) < print_samples) {
            std::cout << "[kernel][find_if][" << i << "]" << " in0=" << in0_ref
    ...
    std::cout << "[kernel][find_if] checked " << n << " elements, mismatches "
              << mismatches << std::endl;
    if (mismatches != 0) {
        aclrtFree(d_in0);
        aclrtFree(d_out0);
