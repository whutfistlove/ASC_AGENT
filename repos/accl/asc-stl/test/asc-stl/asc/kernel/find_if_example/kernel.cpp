...
extern "C" __global__ __aicore__ void find_if_kernel(GM_ADDR in0_gm, GM_ADDR out0_gm)
{
    ...
    for (int32_t tile = 0; tile < TILE_NUM; ++tile) {
        ...
        for (int32_t i = 0; i < TILE_SIZE; ++i) {
            ...
            (void)z_val;
            (void)out0_val;
            { int32_t arr[4] = {2, 4, in0_val, 8};
auto it = asc::std::find_if(arr, arr + 4, [](int32_t v) { return v > 5; });
out0_val = static_cast<int32_t>(it - arr); }
