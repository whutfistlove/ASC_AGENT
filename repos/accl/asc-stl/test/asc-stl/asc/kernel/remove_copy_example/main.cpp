...
#include "acl/acl.h"
#include "host.h"
#include "asc/std/__algorithm/remove_copy.h"
#include <cmath>
#include <cstdlib>
#include <iostream>
#include <vector>
...
int main()
{
    ...
    asc_std_remove_copy_do(8, stream, static_cast<uint8_t*>(d_in0), static_cast<uint8_t*>(d_out0));
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        (void)expected;
        (void)expected0;
        { expected0 = (in0_ref != 5.0f) ? in0_ref : -1.0f; }
        float got0 = h_out0[i];
        if (print_samples < 0 || static_cast<long>(i) < print_samples) {
            std::cout << "[kernel][remove_copy][" << i << "]" << " in0=" << in0_ref
    ...
    std::cout << "[kernel][remove_copy] checked " << n << " elements, mismatches "
              << mismatches << std::endl;
    if (mismatches != 0) {
        aclrtFree(d_in0);
        aclrtFree(d_out0);
