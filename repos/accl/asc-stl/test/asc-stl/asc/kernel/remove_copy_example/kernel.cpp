...
extern "C" __global__ __aicore__ void remove_copy_kernel(GM_ADDR in0_gm, GM_ADDR out0_gm)
{
    ...
    for (int32_t tile = 0; tile < TILE_NUM; ++tile) {
        ...
        for (int32_t i = 0; i < TILE_SIZE; ++i) {
            ...
            (void)z_val;
            (void)out0_val;
            { float val = in0_val; float out_val = -1.0f; asc::std::remove_copy(&val, &val + 1, &out_val, 5.0f); out0_val = out_val; }
