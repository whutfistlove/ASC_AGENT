// auto-workload=full (n=16384, cores=8, tiles=32x64, inputs=3, outputs=3, dtype=int32_t)
#include "kernel_operator.h"
#include "asc/std/__algorithm/sort3.h"

extern "C" __global__ __aicore__ void sort3_kernel(GM_ADDR in0_gm, GM_ADDR in1_gm, GM_ADDR in2_gm, GM_ADDR out0_gm, GM_ADDR out1_gm, GM_ADDR out2_gm)
{
    ...
    AscendC::GlobalTensor<int32_t> out2Gm;
    ...
    out2Gm.SetGlobalBuffer((__gm__ int32_t*)(out2_gm) + block_id * BLOCK_LENGTH, BLOCK_LENGTH);
    ...
    pipe.InitBuffer(outQueue2, 1, TILE_SIZE * sizeof(int32_t));
    ...
    for (int32_t tile = 0; tile < TILE_NUM; ++tile) {
        ...
        auto out2Local = outQueue2.AllocTensor<int32_t>();
        ...
        for (int32_t i = 0; i < TILE_SIZE; ++i) {
            ...
            int32_t out2_val = (int32_t)0;
            int32_t& z_val = out0_val;
            (void)z_val;
            (void)out0_val;
            (void)out1_val;
            (void)out2_val;
            { asc::std::sort3(in0_val, in1_val, in2_val, out0_val, out1_val, out2_val); }
        ...
        auto out2Buf = outQueue2.DeQue<int32_t>();
