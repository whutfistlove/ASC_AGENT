// auto-workload=full (n=16384, cores=8, inputs=3, outputs=3, dtype=int32_t)
#include "acl/acl.h"
#include "host.h"
...
int main()
{
    ...
    asc_std_sort3_do(8, stream, static_cast<uint8_t*>(d_in0), static_cast<uint8_t*>(d_in1), static_cast<uint8_t*>(d_in2), static_cast<uint8_t*>(d_out0), static_cast<uint8_t*>(d_out1), static_cast<uint8_t*>(d_out2));
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        int32_t expected2 = (int32_t)0;
        int32_t& expected = expected0;
        (void)expected;
        (void)expected0;
        (void)expected1;
        (void)expected2;
        { { int32_t a = in0_ref, b = in1_ref, c = in2_ref; if (b < a) { int32_t t = a; a = b; b = t; } if (c < a) { int32_t t = a; a = c; c = t; } if (c < b) { int32_t t = b; b = c; c = t; } expected0 = a; expected1 = b; expected2 = c; } }
        ...
        if (print_samples < 0 || static_cast<long>(i) < print_samples) {
            std::cout << "[kernel][sort3][" << i << "][out1]" << " in0=" << in0_ref << " in1=" << in1_ref << " in2=" << in2_ref
        ...
        int32_t got2 = h_out2[i];
        if (print_samples < 0 || static_cast<long>(i) < print_samples) {
            std::cout << "[kernel][sort3][" << i << "][out2]" << " in0=" << in0_ref << " in1=" << in1_ref << " in2=" << in2_ref
    ...
    std::cout << "[kernel][sort3] checked " << n << " elements, mismatches "
              << mismatches << std::endl;
    if (mismatches != 0) {
        aclrtFree(d_in0);
        aclrtFree(d_in1);
