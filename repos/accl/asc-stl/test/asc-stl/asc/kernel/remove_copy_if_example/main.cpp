// auto-workload=full (n=16384, cores=8, inputs=1, outputs=1, dtype=int32_t)
#include "acl/acl.h"
#include "host.h"
#include "asc/std/__algorithm/remove_copy_if.h"
#include <cmath>
#include <cstdlib>
#include <iostream>
#include <vector>
...
int main()
{
    ...
    auto& h_x = h_in0;
    std::vector<int32_t> h_y(n, (int32_t)0);
    (void)h_x;
    (void)h_y;
    for (size_t i = 0; i < n; ++i) {
        { h_in0[i] = static_cast<int32_t>(i % 5); }
    }
    ...
    asc_std_remove_copy_if_do(8, stream, static_cast<uint8_t*>(d_in0), static_cast<uint8_t*>(d_out0));
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        (void)expected;
        (void)expected0;
        { expected0 = (in0_ref != 2) ? in0_ref : static_cast<int32_t>(-999); }
        int32_t got0 = h_out0[i];
        if (print_samples < 0 || static_cast<long>(i) < print_samples) {
            std::cout << "[kernel][remove_copy_if][" << i << "]" << " in0=" << in0_ref
    ...
    std::cout << "[kernel][remove_copy_if] checked " << n << " elements, mismatches "
              << mismatches << std::endl;
    if (mismatches != 0) {
        aclrtFree(d_in0);
        aclrtFree(d_out0);
