...
extern "C" __global__ __aicore__ void remove_copy_if_kernel(GM_ADDR in0_gm, GM_ADDR out0_gm)
{
    ...
    for (int32_t tile = 0; tile < TILE_NUM; ++tile) {
        ...
        for (int32_t i = 0; i < TILE_SIZE; ++i) {
            ...
            (void)z_val;
            (void)out0_val;
            { int32_t src = in0_val; int32_t dst = -999; asc::std::remove_copy_if(&src, &src + 1, &dst, [](int32_t v){ return v == 2; }); out0_val = dst; }
