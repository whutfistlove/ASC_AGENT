// auto-workload=full (n=16384, cores=8, inputs=4, outputs=5, dtype=float)
#include "acl/acl.h"
#include "host.h"
...
int main()
{
    ...
    for (size_t i = 0; i < n; ++i) {
        { h_in0[i] = static_cast<float>(i % 11) - 5.0f; h_in1[i] = static_cast<float>((i * 3 + 1) % 13) - 6.0f; h_in2[i] = static_cast<float>((i * 7 + 2) % 17) - 8.0f; h_in3[i] = static_cast<float>((i * 11 + 3) % 19) - 9.0f; }
    }
    ...
    asc_std_quad_fanout_do(8, stream, static_cast<uint8_t*>(d_in0), static_cast<uint8_t*>(d_in1), static_cast<uint8_t*>(d_in2), static_cast<uint8_t*>(d_in3), static_cast<uint8_t*>(d_out0), static_cast<uint8_t*>(d_out1), static_cast<uint8_t*>(d_out2), static_cast<uint8_t*>(d_out3), static_cast<uint8_t*>(d_out4));
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        float expected2 = (float)0;
        float expected3 = (float)0;
        float expected4 = (float)0;
