...
int main()
{
    ...
    for (size_t i = 0; i < n; ++i) {
        { h_in0[i] = static_cast<int32_t>(i % 7); }
    }
    ...
    asc_std_replace_do(8, stream, static_cast<uint8_t*>(d_in0), static_cast<uint8_t*>(d_out0));
    ...
    std::cout << "[kernel][replace] checked " << n << " elements, mismatches "
              << mismatches << std::endl;
    if (mismatches != 0) {
        aclrtFree(d_in0);
        aclrtFree(d_out0);
