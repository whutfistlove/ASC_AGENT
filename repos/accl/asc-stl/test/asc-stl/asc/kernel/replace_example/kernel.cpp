...
extern "C" __global__ __aicore__ void replace_kernel(GM_ADDR in0_gm, GM_ADDR out0_gm)
{
    ...
    for (int32_t tile = 0; tile < TILE_NUM; ++tile) {
        ...
        for (int32_t i = 0; i < TILE_SIZE; ++i) {
            ...
            (void)z_val;
            (void)out0_val;
            { int32_t elem = in0_val; asc::std::replace(&elem, &elem + 1, int32_t(2), int32_t(5)); out0_val = elem; }
