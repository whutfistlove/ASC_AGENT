...
int main()
{
    ...
    for (size_t i = 0; i < n; ++i) {
        { h_in0[i] = static_cast<int32_t>((i * 7 + 3) % 19) - 9; h_in1[i] = static_cast<int32_t>((i * 11 + 5) % 23) - 11; }
    }
    ...
    asc_std_find_if_not_do(8, stream, static_cast<uint8_t*>(d_in0), static_cast<uint8_t*>(d_in1), static_cast<uint8_t*>(d_out0));
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        (void)expected;
        (void)expected0;
        { int32_t arr[4] = {in0_ref, in0_ref + 1, in1_ref, in1_ref + 1};
int32_t* it = arr;
for (; it != arr + 4; ++it) { if (!(*it % 2 == 0)) break; }
expected0 = (it != arr + 4) ? *it : static_cast<int32_t>(-1); }
        int32_t got0 = h_out0[i];
        if (print_samples < 0 || static_cast<long>(i) < print_samples) {
            std::cout << "[kernel][find_if_not][" << i << "]" << " in0=" << in0_ref << " in1=" << in1_ref
    ...
    std::cout << "[kernel][find_if_not] checked " << n << " elements, mismatches "
              << mismatches << std::endl;
    if (mismatches != 0) {
        aclrtFree(d_in0);
        aclrtFree(d_in1);
