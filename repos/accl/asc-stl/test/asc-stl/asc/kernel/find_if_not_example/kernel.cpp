// auto-workload=full (n=16384, cores=8, tiles=32x64, inputs=2, outputs=1, dtype=int32_t)
#include "kernel_operator.h"
#include "asc/std/__algorithm/find_if_not.h"

extern "C" __global__ __aicore__ void find_if_not_kernel(GM_ADDR in0_gm, GM_ADDR in1_gm, GM_ADDR out0_gm)
{
    ...
    for (int32_t tile = 0; tile < TILE_NUM; ++tile) {
        ...
        for (int32_t i = 0; i < TILE_SIZE; ++i) {
            ...
            (void)z_val;
            (void)out0_val;
            { int32_t arr[4] = {in0_val, in0_val + 1, in1_val, in1_val + 1};
auto it = asc::std::find_if_not(arr, arr + 4, [](int32_t v) { return v % 2 == 0; });
out0_val = (it != arr + 4) ? *it : static_cast<int32_t>(-1); }
