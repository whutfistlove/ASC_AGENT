// auto-workload=fast (n=64, cores=1, tiles=1x64, inputs=2, outputs=2, dtype=float)
#include "kernel_operator.h"
#include "asc/std/__algorithm/minmax.h"

extern "C" __global__ __aicore__ void minmax_kernel(GM_ADDR in0_gm, GM_ADDR in1_gm, GM_ADDR out0_gm, GM_ADDR out1_gm)
{
    ...
    for (int32_t tile = 0; tile < TILE_NUM; ++tile) {
        ...
        for (int32_t i = 0; i < TILE_SIZE; ++i) {
            ...
            (void)out0_val;
            (void)out1_val;
            { auto pr = asc::std::minmax(in0_val, in1_val); out0_val = pr.first; out1_val = pr.second; }
