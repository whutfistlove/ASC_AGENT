...
int main()
{
    ...
    asc_std_views_do(8, stream, static_cast<uint8_t*>(d_in0), static_cast<uint8_t*>(d_out0));
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        if (print_samples < 0 || static_cast<long>(i) < print_samples) {
            std::cout << "[kernel][views][" << i << "]" << " in0=" << in0_ref
    ...
    std::cout << "[kernel][views] checked " << n << " elements, mismatches "
              << mismatches << std::endl;
    if (mismatches != 0) {
        aclrtFree(d_in0);
        aclrtFree(d_out0);
