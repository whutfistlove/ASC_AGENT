...
int main()
{
    ...
    asc_std_all_of_do(8, stream, static_cast<uint8_t*>(d_in0), static_cast<uint8_t*>(d_in1), static_cast<uint8_t*>(d_in2), static_cast<uint8_t*>(d_in3), static_cast<uint8_t*>(d_out0));
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        (void)expected;
        (void)expected0;
        { int32_t refs[4] = {in0_ref, in1_ref, in2_ref, in3_ref}; bool all_even = true; for (int j = 0; j < 4; ++j) { if (refs[j] % 2 != 0) { all_even = false; break; } } expected0 = all_even ? 1 : 0; }
        int32_t got0 = h_out0[i];
        if (print_samples < 0 || static_cast<long>(i) < print_samples) {
            std::cout << "[kernel][all_of][" << i << "]" << " in0=" << in0_ref << " in1=" << in1_ref << " in2=" << in2_ref << " in3=" << in3_ref
    ...
    std::cout << "[kernel][all_of] checked " << n << " elements, mismatches "
              << mismatches << std::endl;
    if (mismatches != 0) {
        aclrtFree(d_in0);
        aclrtFree(d_in1);
