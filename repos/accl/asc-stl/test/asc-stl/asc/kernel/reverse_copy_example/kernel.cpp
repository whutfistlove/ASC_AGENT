...
extern "C" __global__ __aicore__ void reverse_copy_kernel(GM_ADDR in0_gm, GM_ADDR in1_gm, GM_ADDR out0_gm, GM_ADDR out1_gm)
{
    ...
    for (int32_t tile = 0; tile < TILE_NUM; ++tile) {
        ...
        for (int32_t i = 0; i < TILE_SIZE; ++i) {
            ...
            (void)out0_val;
            (void)out1_val;
            { float src[2] = {in0_val, in1_val}; float dst[2] = {0, 0}; asc::std::reverse_copy(src, src + 2, dst); out0_val = dst[0]; out1_val = dst[1]; }
