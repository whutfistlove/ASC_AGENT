...
int main()
{
    ...
    for (size_t i = 0; i < n; ++i) {
        { h_in0[i] = static_cast<float>(i % 97) - 40.0f; h_in1[i] = static_cast<float>((i * 7) % 97) - 50.0f; }
    }
    ...
    asc_std_reverse_copy_do(8, stream, static_cast<uint8_t*>(d_in0), static_cast<uint8_t*>(d_in1), static_cast<uint8_t*>(d_out0), static_cast<uint8_t*>(d_out1));
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        (void)expected0;
        (void)expected1;
        { expected0 = in1_ref; expected1 = in0_ref; }
        float got0 = h_out0[i];
        if (print_samples < 0 || static_cast<long>(i) < print_samples) {
            std::cout << "[kernel][reverse_copy][" << i << "][out0]" << " in0=" << in0_ref << " in1=" << in1_ref
        ...
        float got1 = h_out1[i];
        if (print_samples < 0 || static_cast<long>(i) < print_samples) {
            std::cout << "[kernel][reverse_copy][" << i << "][out1]" << " in0=" << in0_ref << " in1=" << in1_ref
    ...
    std::cout << "[kernel][reverse_copy] checked " << n << " elements, mismatches "
              << mismatches << std::endl;
    if (mismatches != 0) {
        aclrtFree(d_in0);
        aclrtFree(d_in1);
