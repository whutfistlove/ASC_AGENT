// auto-workload=full (n=16384, cores=8, inputs=3, outputs=1, dtype=float)
#include "acl/acl.h"
#include "host.h"
...
int main()
{
    ...
    for (size_t i = 0; i < n; ++i) {
        { h_in0[i] = static_cast<float>((i * 7 + 3) % 13) - 6.0f; h_in1[i] = static_cast<float>((i * 11 + 1) % 13) - 6.0f; h_in2[i] = static_cast<float>((i * 17 + 5) % 13) - 6.0f; }
    }
    ...
    asc_std_median3_do(8, stream, static_cast<uint8_t*>(d_in0), static_cast<uint8_t*>(d_in1), static_cast<uint8_t*>(d_in2), static_cast<uint8_t*>(d_out0));
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        (void)expected;
        (void)expected0;
        { float lo = in0_ref < in1_ref ? (in0_ref < in2_ref ? in0_ref : in2_ref) : (in1_ref < in2_ref ? in1_ref : in2_ref);
float hi = in0_ref > in1_ref ? (in0_ref > in2_ref ? in0_ref : in2_ref) : (in1_ref > in2_ref ? in1_ref : in2_ref);
expected0 = in0_ref + in1_ref + in2_ref - lo - hi; }
        float got0 = h_out0[i];
        if (print_samples < 0 || static_cast<long>(i) < print_samples) {
            std::cout << "[kernel][median3][" << i << "]" << " in0=" << in0_ref << " in1=" << in1_ref << " in2=" << in2_ref
    ...
    std::cout << "[kernel][median3] checked " << n << " elements, mismatches "
              << mismatches << std::endl;
    if (mismatches != 0) {
        aclrtFree(d_in0);
        aclrtFree(d_in1);
