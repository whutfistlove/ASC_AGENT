...
extern "C" __global__ __aicore__ void median3_kernel(GM_ADDR in0_gm, GM_ADDR in1_gm, GM_ADDR in2_gm, GM_ADDR out0_gm)
{
    ...
    for (int32_t tile = 0; tile < TILE_NUM; ++tile) {
        ...
        for (int32_t i = 0; i < TILE_SIZE; ++i) {
            ...
            (void)z_val;
            (void)out0_val;
            { out0_val = asc::std::median3(in0_val, in1_val, in2_val); }
