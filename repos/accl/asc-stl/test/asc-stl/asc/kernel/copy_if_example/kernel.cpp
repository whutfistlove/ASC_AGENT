...
extern "C" __global__ __aicore__ void copy_if_kernel(GM_ADDR in0_gm, GM_ADDR out0_gm)
{
    ...
    for (int32_t tile = 0; tile < TILE_NUM; ++tile) {
        ...
        for (int32_t i = 0; i < TILE_SIZE; ++i) {
            ...
            (void)z_val;
            (void)out0_val;
            { int a = in0_val; int b = 0; asc::std::copy_if(&a, &a + 1, &b, [](int v){ return v % 3 == 0; }); out0_val = b; }
