...
int main()
{
    ...
    for (size_t i = 0; i < n; ++i) {
        { h_x[i] = static_cast<float>((i * 7) % 100) - 30.0f; }
    }
    ...
    asc_std_replace_copy_if_do(8, stream, static_cast<uint8_t*>(d_in0), static_cast<uint8_t*>(d_out0));
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        (void)expected;
        (void)expected0;
        { expected = (x_ref < 0.0f) ? 0.0f : x_ref; }
        float got0 = h_out0[i];
        if (print_samples < 0 || static_cast<long>(i) < print_samples) {
            std::cout << "[kernel][replace_copy_if][" << i << "]" << " in0=" << in0_ref
    ...
    std::cout << "[kernel][replace_copy_if] checked " << n << " elements, mismatches "
              << mismatches << std::endl;
    if (mismatches != 0) {
        aclrtFree(d_in0);
        aclrtFree(d_out0);
