...
int main()
{
    ...
    for (size_t i = 0; i < n; ++i) {
        { h_in0[i] = (i % 2 == 0) ? static_cast<float>(i + 1) : -static_cast<float>(i + 1); }
    }
    ...
    asc_std_any_of_do(8, stream, static_cast<uint8_t*>(d_in0), static_cast<uint8_t*>(d_out0));
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        (void)expected;
        (void)expected0;
        { float val_ref = in0_ref;
bool expected_res = val_ref > 0.0f;
expected0 = expected_res ? 1.0f : 0.0f; }
        float got0 = h_out0[i];
        if (print_samples < 0 || static_cast<long>(i) < print_samples) {
            std::cout << "[kernel][any_of][" << i << "]" << " in0=" << in0_ref
    ...
    std::cout << "[kernel][any_of] checked " << n << " elements, mismatches "
              << mismatches << std::endl;
    if (mismatches != 0) {
        aclrtFree(d_in0);
        aclrtFree(d_out0);
