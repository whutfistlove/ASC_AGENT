// auto-workload=full (n=16384, cores=8, inputs=4, outputs=1, dtype=int32_t)
#include "acl/acl.h"
#include "host.h"
...
int main()
{
    ...
    for (size_t i = 0; i < n; ++i) {
        { h_in0[i] = static_cast<int32_t>((i * 2) % 20); h_in1[i] = static_cast<int32_t>(((i * 3 + 1) % 20)); h_in2[i] = static_cast<int32_t>(((i * 5 + 2) % 20)); h_in3[i] = static_cast<int32_t>(((i * 7 + 3) % 20)); }
    }
    ...
    asc_std_none_of_do(8, stream, static_cast<uint8_t*>(d_in0), static_cast<uint8_t*>(d_in1), static_cast<uint8_t*>(d_in2), static_cast<uint8_t*>(d_in3), static_cast<uint8_t*>(d_out0));
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        (void)expected;
        (void)expected0;
        { int32_t refs[4] = {in0_ref, in1_ref, in2_ref, in3_ref}; bool none_even = true; for (int j = 0; j < 4; ++j) { if (refs[j] % 2 == 0) { none_even = false; break; } } expected0 = none_even ? 1 : 0; }
        int32_t got0 = h_out0[i];
        if (print_samples < 0 || static_cast<long>(i) < print_samples) {
            std::cout << "[kernel][none_of][" << i << "]" << " in0=" << in0_ref << " in1=" << in1_ref << " in2=" << in2_ref << " in3=" << in3_ref
    ...
    std::cout << "[kernel][none_of] checked " << n << " elements, mismatches "
              << mismatches << std::endl;
    if (mismatches != 0) {
        aclrtFree(d_in0);
        aclrtFree(d_in1);
