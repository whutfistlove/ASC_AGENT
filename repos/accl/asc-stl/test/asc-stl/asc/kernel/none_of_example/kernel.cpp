...
extern "C" __global__ __aicore__ void none_of_kernel(GM_ADDR in0_gm, GM_ADDR in1_gm, GM_ADDR in2_gm, GM_ADDR in3_gm, GM_ADDR out0_gm)
{
    ...
    for (int32_t tile = 0; tile < TILE_NUM; ++tile) {
        ...
        for (int32_t i = 0; i < TILE_SIZE; ++i) {
            ...
            (void)z_val;
            (void)out0_val;
            { int32_t arr[4] = {in0_val, in1_val, in2_val, in3_val}; auto is_even = [](const int32_t& v) { return v % 2 == 0; }; out0_val = asc::std::none_of(arr, arr + 4, is_even) ? 1 : 0; }
