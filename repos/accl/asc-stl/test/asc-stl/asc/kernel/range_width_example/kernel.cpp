// auto-workload=full (n=16384, cores=8, tiles=32x64, inputs=3, outputs=1, dtype=float)
#include "kernel_operator.h"
#include "asc/std/__numeric/range_width.h"

extern "C" __global__ __aicore__ void range_width_kernel(GM_ADDR in0_gm, GM_ADDR in1_gm, GM_ADDR in2_gm, GM_ADDR out0_gm)
{
    ...
    for (int32_t tile = 0; tile < TILE_NUM; ++tile) {
        ...
        for (int32_t i = 0; i < TILE_SIZE; ++i) {
            ...
            (void)z_val;
            (void)out0_val;
            { out0_val = asc::std::range_width(in0_val, in1_val, in2_val); }
