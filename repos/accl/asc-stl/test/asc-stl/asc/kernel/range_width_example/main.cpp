...
int main()
{
    ...
    for (size_t i = 0; i < n; ++i) {
        { h_in0[i] = static_cast<float>(i % 11) - 5.0f; h_in1[i] = static_cast<float>((i * 3) % 13) - 6.0f; h_in2[i] = static_cast<float>((i * 7) % 17) - 8.0f; }
    }
    ...
    asc_std_range_width_do(8, stream, static_cast<uint8_t*>(d_in0), static_cast<uint8_t*>(d_in1), static_cast<uint8_t*>(d_in2), static_cast<uint8_t*>(d_out0));
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        (void)expected;
        (void)expected0;
        { float mx = in0_ref; if (in1_ref > mx) mx = in1_ref; if (in2_ref > mx) mx = in2_ref; float mn = in0_ref; if (in1_ref < mn) mn = in1_ref; if (in2_ref < mn) mn = in2_ref; expected0 = mx - mn; }
        float got0 = h_out0[i];
        if (print_samples < 0 || static_cast<long>(i) < print_samples) {
            std::cout << "[kernel][range_width][" << i << "]" << " in0=" << in0_ref << " in1=" << in1_ref << " in2=" << in2_ref
    ...
    std::cout << "[kernel][range_width] checked " << n << " elements, mismatches "
              << mismatches << std::endl;
    if (mismatches != 0) {
        aclrtFree(d_in0);
        aclrtFree(d_in1);
