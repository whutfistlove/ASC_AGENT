...
extern "C" __global__ __aicore__ void default_sentinel_kernel(GM_ADDR in0_gm, GM_ADDR out0_gm)
{
    ...
    for (int32_t tile = 0; tile < TILE_NUM; ++tile) {
        ...
        for (int32_t i = 0; i < TILE_SIZE; ++i) {
            ...
            (void)z_val;
            (void)out0_val;
            { asc::std::default_sentinel_t s = asc::std::default_sentinel; (void)s; z_val = 1.0f; }
