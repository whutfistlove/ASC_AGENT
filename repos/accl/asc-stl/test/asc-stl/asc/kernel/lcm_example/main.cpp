// auto-workload=full (n=16384, cores=8, inputs=2, outputs=1, dtype=int64_t)
#include "acl/acl.h"
#include "host.h"
...
int main()
{
    ...
    std::vector<int64_t> h_in0(n);
    std::vector<int64_t> h_in1(n);
    std::vector<int64_t> h_out0(n);
    auto& h_x = h_in0;
    auto& h_y = h_in1;
    (void)h_x;
    (void)h_y;
    for (size_t i = 0; i < n; ++i) {
        { h_in0[i] = static_cast<int64_t>(i % 20) - 10; h_in1[i] = static_cast<int64_t>((i * 3 + 5) % 20) - 10; }
    }
    ...
    asc_std_lcm_do(8, stream, static_cast<uint8_t*>(d_in0), static_cast<uint8_t*>(d_in1), static_cast<uint8_t*>(d_out0));
    ...
    size_t mismatches = 0;
    for (size_t i = 0; i < n; ++i) {
        int64_t in0_ref = h_in0[i];
        int64_t in1_ref = h_in1[i];
        int64_t x_ref = in0_ref;
        int64_t y_ref = in1_ref;
        (void)x_ref;
        (void)y_ref;
        (void)in0_ref;
        (void)in1_ref;
        int64_t expected0 = (int64_t)0;
        int64_t& expected = expected0;
        (void)expected;
        (void)expected0;
        { int64_t a_ref = (x_ref < 0) ? -x_ref : x_ref;
int64_t b_ref = (y_ref < 0) ? -y_ref : y_ref;
if (a_ref == 0 || b_ref == 0) {
    expected0 = 0;
} else {
    int64_t ta = a_ref, tb = b_ref;
    while (tb != 0) {
        int64_t r = ta % tb;
        ta = tb;
        tb = r;
    }
    expected0 = (a_ref / ta) * b_ref;
} }
        int64_t got0 = h_out0[i];
        if (print_samples < 0 || static_cast<long>(i) < print_samples) {
            std::cout << "[kernel][lcm][" << i << "]" << " in0=" << in0_ref << " in1=" << in1_ref
    ...
    std::cout << "[kernel][lcm] checked " << n << " elements, mismatches "
              << mismatches << std::endl;
    if (mismatches != 0) {
        aclrtFree(d_in0);
        aclrtFree(d_in1);
