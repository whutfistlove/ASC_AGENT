...
int main()
{
    ...
    expect_bool(
        "none_of evens even",
        asc::std::none_of(evens, evens + 4, IsEven()),
        independent_none_of(evens, IsEven()));
    expect_bool(
        "none_of mixed even",
        asc::std::none_of(mixed, mixed + 4, IsEven()),
        independent_none_of(mixed, IsEven()));
    expect_bool(
        "none_of odds even",
        asc::std::none_of(odds, odds + 4, IsEven()),
        independent_none_of(odds, IsEven()));
    expect_bool(
        "none_of negative positive",
        asc::std::none_of(negative, negative + 4, IsPositive()),
        independent_none_of(negative, IsPositive()));
    expect_bool(
        "none_of empty range",
        asc::std::none_of(empty, empty, IsEven()),
        true);

    constexpr int constexpr_values[] = {1, 3, 5, 7};
    static_assert(asc::std::none_of(constexpr_values, constexpr_values + 4, IsEven()), "");
