...
int main()
{
    ...
    expect_index(
        ...
        asc::std::find_if(values, values + 4, Equals{2}),
        values,
        independent_find_if(values, values + 4, Equals{2}));
    expect_index(
        "middle element matches",
        asc::std::find_if(values, values + 4, Equals{6}),
        values,
        independent_find_if(values, values + 4, Equals{6}));
    expect_index(
        "last element matches",
        asc::std::find_if(values, values + 4, Equals{8}),
        values,
        independent_find_if(values, values + 4, Equals{8}));
    expect_index(
        "first repeated match",
        asc::std::find_if(repeated, repeated + 4, Equals{4}),
        repeated,
        independent_find_if(repeated, repeated + 4, Equals{4}));
    expect_index(
        "no element matches",
        asc::std::find_if(values, values + 4, Equals{10}),
        values,
        independent_find_if(values, values + 4, Equals{10}));
    expect_index(
        "empty range",
        asc::std::find_if(values, values, IsEven()),
        values,
        values);
    expect_index(
        "odds even",
        asc::std::find_if(odds, odds + 4, IsEven()),
        odds,
        independent_find_if(odds, odds + 4, IsEven()));

    constexpr int constexpr_values[] = {1, 3, 4, 7};
    static_assert(asc::std::find_if(constexpr_values, constexpr_values + 4, IsEven()) == constexpr_values + 2, "");
